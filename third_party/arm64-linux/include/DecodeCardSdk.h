...
DLLEXPORT_API int __stdcall StartVideoPreview(
    HANDLE hChannelHandle, HWND WndHandle, RECT *rect, BOOLEAN bOverlay, int VideoFormat, int FrameRate);
...
DLLEXPORT_API int __stdcall GetVideoPara(
    HANDLE hChannelHandle, VideoStandard_t *VideoStandard, int *Brightness, int *Contrast, int *Saturation, int *Hue);
...
DLLEXPORT_API int __stdcall SetOsdDisplayMode(
    HANDLE hChannelHandle, int Brightness, BOOL Translucent, int parameter, USHORT *Format1, USHORT *Format2);
...
/*
    锟斤拷锟斤拷为1.7锟芥本锟斤拷锟斤拷锟侥癸拷锟杰★拷
    锟皆猴拷锟斤拷芑锟斤拷锟斤拷锟矫伙拷锟侥凤拷锟斤拷锟斤拷锟斤拷锟睫改猴拷锟斤拷锟狡★拷
*/
// 原始图锟斤拷锟斤拷锟斤拷锟斤拷
...
/*
    锟斤拷锟斤拷锟斤拷频锟斤拷锟斤拷位锟矫ｏ拷
        (x,y)为系统锟斤拷锟斤拷锟斤拷图锟斤拷锟斤拷锟斤拷辖锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟皆�始图锟斤拷锟叫碉拷锟斤拷锟疥。
        x锟斤拷锟斤拷为2锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷
        (x,y)锟斤拷锟斤拷牟锟斤拷锟斤拷锟轿э拷锟斤拷锟斤拷锟斤拷锟斤拷锟酵猴拷锟叫关ｏ拷锟斤拷锟街革拷锟斤拷锟街碉拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟诫不匹锟戒，
    锟斤拷锟杰会导锟斤拷图锟斤拷止锟斤拷水平锟斤拷锟斤拷直锟斤拷锟斤拷锟斤拷锟斤拷锟�
*/
...
#define SERIAL_NUMBER_LENGTH 12 // 鏉垮崱搴忓垪鍙烽暱搴�
#endif 12                       // 锟藉卡锟斤拷锟叫号筹拷锟斤拷
typedef struct tagDS_BOARD_DETAIL
{
    BOARD_TYPE_DS type;            // 锟藉卡锟斤拷锟斤拷
    BYTE sn[16];                   // 锟斤拷锟叫猴拷
    UINT dspCount;                 // 锟藉卡锟斤拷锟斤拷锟斤拷DSP锟斤拷锟斤拷
    UINT firstDspIndex;            // 锟藉卡锟较碉拷一锟斤拷DSP锟斤拷锟斤拷锟斤拷
    UINT encodeChannelCount;       // 锟藉卡锟斤拷锟斤拷锟侥憋拷锟斤拷通锟斤拷锟斤拷锟斤拷
    UINT firstEncodeChannelIndex;  // 锟藉卡锟较碉拷一锟斤拷锟斤拷锟斤拷通锟斤拷锟斤拷锟斤拷锟斤拷
    UINT decodeChannelCount;       // 锟藉卡锟斤拷锟斤拷锟侥斤拷锟斤拷通锟斤拷锟斤拷锟斤拷
    UINT firstDecodeChannelIndex;  // 锟藉卡锟较碉拷一锟斤拷锟斤拷锟斤拷通锟斤拷锟斤拷锟斤拷锟斤拷
    UINT displayChannelCount;      // 锟藉卡锟斤拷锟斤拷锟斤拷锟斤拷频锟斤拷锟酵�锟斤拷锟斤拷锟斤拷
    UINT firstDisplayChannelIndex; // 锟藉卡锟较碉拷一锟斤拷锟斤拷频锟斤拷锟酵�锟斤拷锟斤拷锟斤拷锟斤拷
    UINT reserved1;
    UINT reserved2;
    UINT reserved3;
    UINT reserved4;
...
DLLEXPORT_API int __stdcall SetEncoderVideoExtOutput(
    UINT nEncodeChannel, UINT nPort, BOOL bOpen, UINT nDisplayChannel, UINT nDisplayRegion, UINT nReserved);
DLLEXPORT_API int __stdcall SetDecoderVideoExtOutput(
    UINT nDecodeChannel, UINT nPort, BOOL bOpen, UINT nDisplayChannel, UINT nDisplayRegion, UINT nReserved);
DLLEXPORT_API int __stdcall SetDecoderVideoOutput(
    UINT nDecodeChannel, UINT nPort, BOOL bOpen, UINT nDisplayChannel, UINT nDisplayRegion, UINT nReserved);
...
// 4.0
PLAYER_API int __stdcall SetOsdDisplayModeEx(
    HANDLE hChannelHandle, int color, BOOL Translucent, int param, int nLineCount, USHORT **Format);
typedef void (*MOTION_DETECTION_CALLBACK)(ULONG channelNumber, BOOL bMotionDetected, void *context);
PLAYER_API int __stdcall SetupMotionDetectionEx(HANDLE hChannelHandle,
                                                int iGrade,
                                                int iFastMotionDetectFps,
                                                int iSlowMotionDetectFps,
                                                UINT delay,
                                                RECT *RectList,
                                                int iAreas,
                                                MOTION_DETECTION_CALLBACK MotionDetectionCallback,
                                                int reserved);
...
DLLEXPORT_API int __stdcall SetDisplayVideoCapture(
    UINT nDisplayChannel, BOOL bStart, UINT fps, UINT width, UINT height, unsigned char *imageBuffer);
...
/*
锟斤拷锟斤拷锟斤拷锟接匡拷:锟斤拷锟斤拷锟斤拷锟皆★拷锟斤拷锟斤拷锟斤拷锟皆猴拷锟斤拷芑锟斤拷锟斤拷薷摹锟�
*/
typedef struct
{
...
typedef void (*FACE_DETECTION_DEMO_CALLBACK)(
    UINT nChannel, UINT nFaceCount, FACE_INFO_DEMO *pFaceInfo, char *pData, UINT nDataSize, UINT nImageWidth, UINT nImageHeight);

DLLEXPORT_API int __stdcall SetFaceDetectionDemo(HANDLE hChannelHandle,
                                                 BOOL bEnable,
                                                 UINT nFrameInterval,
                                                 FACE_DETECTION_DEMO_CALLBACK pFunc,
                                                 BOOL bCompress,
                                                 UINT nCompressQuality,
                                                 BOOL bLocateEyePos);
