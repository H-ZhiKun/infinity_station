...
#define SERIAL_NUMBER_LENGTH 12 // ∞Âø®–Ú¡–∫≈≥§∂»
#endif 12                       // ?èÒ???ß‹????
typedef struct tagDS_BOARD_DETAIL
{
    BOARD_TYPE_DS type;            // ?èÒ????
    BYTE sn[16];                   // ???ß‹?
    UINT dspCount;                 // ?èÒ??????DSP????
    UINT firstDspIndex;            // ?èÒ??????DSP??????
    UINT encodeChannelCount;       // ?èÒ????????????????
    UINT firstEncodeChannelIndex;  // ?èÒ???????????????????
    UINT decodeChannelCount;       // ?èÒ????????????????
    UINT firstDecodeChannelIndex;  // ?èÒ???????????????????
    UINT displayChannelCount;      // ?èÒ???????????????????
    UINT firstDisplayChannelIndex; // ?èÒ?????????????????????
    UINT reserved1;
    UINT reserved2;
    UINT reserved3;
    UINT reserved4;
