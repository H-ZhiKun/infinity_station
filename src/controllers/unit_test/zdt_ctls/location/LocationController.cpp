...
void LocationController::sendLocationStatus(bool status)
{
    App().invokeModuleAsync(TIS_Info::QmlPrivateEngine::callFromCpp, TIS_Info::QmlCommunication::ForQmlSignals::location_state, status);
}
...
void LocationController::doDisplay(const QVariant &data)
{
    ...
    _Kits::LogDebug(
        "stationName:{},poleName:{},dis:{},locationType:{}", stationName.toStdString(), poleName.toStdString(), dis, location_type);
}
