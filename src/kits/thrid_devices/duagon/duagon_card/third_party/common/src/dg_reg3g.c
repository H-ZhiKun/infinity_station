...
#if (1 == HDIO_DEBUG)
#define hdreg3_debug_printf osl_printf
#else
#define hdreg3_debug_printf dummy_printf
...
void dg_reg3g_set_perf_callbacks(PerfFunc write, PerfFunc read_d, PerfFunc read_s, PerfFuncU8 polling, DG_U8 card_index) {
    dg_card_config[card_index].p3g.write_func = write;
    dg_card_config[card_index].p3g.read_d_func = read_d;
    dg_card_config[card_index].p3g.read_s_func = read_s;
    dg_card_config[card_index].p3g.is_polling_func = polling;
}
...
{
    DG_RESULT res = DG_OK;
    UE3G_REGISTER_MAP* base = (UE3G_REGISTER_MAP*)GET_REG_BASE(card_index,channel_num);
    OSL_TIMER  t1;
    DG_U32 bytes_read = 0;
    DG_BOOL status;
    ...
    if ( DG_CARD_IO_MODE(card_index)  == DG_HDCFG_USB ) {
        if (DG_HDCFG_BLOCKING_BLOCKING == dg_card_config[card_index].dg_hdio_configs[channel_num].blocking) {
            hdreg3_debug_printf("dg_reg_receive_3g_blocking\n");
            ...
            hdreg3_debug_printf("dg_reg_receive_3g_non_blocking\n");
    ...
    if ( DG_CARD_IO_MODE(card_index) == DG_HDCFG_PARALLEL ) {
        ...
        base = (UE3G_REGISTER_MAP*)GET_REG_BASE(card_index,channel_num);
         ...
         if (status) {
            ...
            /* busy wait */
                ...
                *rxBuff = osl_par_hal_read8((REG_SIZE*)(&(base->io)));
                ...
                bytes_read++;
#endif
                if (dg_card_config[card_index].p3g.read_d_func!=NULL) {
                    dg_card_config[card_index].p3g.read_d_func(channel_num, card_index);
                }
             }
         } else {
...
)
{
    ...
    if  ( DG_CARD_IO_MODE(card_index) == DG_HDCFG_PARALLEL ) {
        ...
        while(curr_size<(*size)){
            ...
            curr_size++;
#endif
            if (dg_card_config[card_index].p3g.write_func!=NULL) {
                dg_card_config[card_index].p3g.write_func(channel_num, card_index);
            }
        }
...
{
    DG_BOOL res = DG_OK;
    hdreg3_debug_printf("dg_reg3_tx_status_check 0x%08X\n", (DG_U32)addr);
#ifdef USE_OLD_UE3G_REGISTER_MAP
    res = (DG_BOOL)((hd_reg_get_status(addr, card_index) & 0x80) != 0);
#else
    res = (DG_BOOL)((hd_reg_get_status(addr, card_index) & 0x40) != 0);
#endif
    if (dg_card_config[card_index].p3g.read_s_func!=NULL) {
        dg_card_config[card_index].p3g.read_s_func(MVB_PROTOCOL_CHANNEL, card_index);
    }
    return res;
}
...
)
{
    ...
    hdreg3_debug_printf("dg_reg3_wait_for_status\n");
    osl_start_timer(&t1);
    while (!dg_reg_rx_status_check(addr, card_index)) {
        if (!osl_check_timer(&t1, timeout)) {
            hdreg3_debug_printf("dg_reg3_wait_for_status status TIMEOUT\n");
            return DG_ERROR_TIMEOUT;
        }
    }
    hdreg3_debug_printf("dg_reg3_wait_for_status status OK\n");
    return DG_OK;
}
