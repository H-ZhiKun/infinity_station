#include "SaveVideoQImage.h"
#include <QDebug>
#include <filesystem>

namespace _Kits::SaveVideoNew
{
    SaveVideoQImage::SaveVideoQImage(QObject *parent) : m_muxingVideo(nullptr), m_encodeVideo(nullptr), m_isrecordvideo(false)
    {
    }

    SaveVideoQImage::~SaveVideoQImage()
    {
    ...
    int SaveVideoQImage::initSaveVideo(const int &threadNum)
    {
        m_videoinputinfo.avcodeid = AV_CODEC_ID_H264;
        m_videoinputinfo.bit_rate = 2500000;
        m_videoinputinfo.width = 1920;
        m_videoinputinfo.height = 1080;
        ...
        {
            /* code */
            m_vec_thread_record.emplace_back(std::thread(&SaveVideoQImage::threadRecoed, this));
        }
        return 0;
    }
    void SaveVideoQImage::stopRecord()
    {
    ...
    int SaveVideoQImage::startRecord(QString &filepath)
    {
        if (nullptr != m_encodeVideo && nullptr != m_muxingVideo)
        {
            stopRecord();
            ...
            auto writecallfunck =
                std::bind(&SaveVideoQImage::writeVideofunc, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
            ...
            //  auto filepath = m_cameraInfo.mstr_Video_SavePath + "/" + m_cameraInfo.mstr_CameraName + "/" + filename + "." +
    ...
    void SaveVideoQImage::writeVideofunc(int no, AVPacket &pkt, int flag)
    {
    ...
    void SaveVideoQImage::pushImage(const QImage &img)
    {
    ...
    void SaveVideoQImage::threadRecoed()
    {
