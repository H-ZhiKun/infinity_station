...
void MuxingVideo::AddStream(std::shared_ptr<OutputStream> optstream, EncodeVideo::InputInfo &inputinfo)
{
    ...
    {
        qDebug("Could not find encoder for %s", avcodec_get_name((AVCodecID)inputinfo.avcodeid));
        return;
    }
...
void MuxingVideo::RKAddStream(std::shared_ptr<OutputStream> optstream, EncodeVideo::InputInfo &inputinfo)
{
